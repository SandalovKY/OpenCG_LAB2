    ...
    class View
    {
        ...
        public void DrawQuadStrip (int layerNumber)
        {
            ...
            for (int y_coord = 0; y_coord < Bin.Y - 2; y_coord+=2)
            {
                    ...
                    {
                        val = Bin.array[x_coord + (y_coord) * Bin.X + layerNumber * Bin.X * Bin.Y];
                        GL.Color3(TransferFunction(val));
                        GL.Vertex2(x_coord, y_coord);

                        val = Bin.array[x_coord + (y_coord + 1) * Bin.X + layerNumber * Bin.X * Bin.Y];
                        GL.Color3(TransferFunction(val));
                        GL.Vertex2(x_coord, y_coord + 1);
                    }
                    for (int x_coord = Bin.X - 1; x_coord >= 0; --x_coord)
                    {
                        ...
                        val = Bin.array[x_coord + (y_coord + 2) * Bin.X + layerNumber * Bin.X * Bin.Y];
                        GL.Color3(TransferFunction(val));
            ...
            for (int x_coord = 0; x_coord < Bin.X; ++x_coord)
            {
                val = Bin.array[x_coord + (Bin.Y - 2) * Bin.X + layerNumber * Bin.X * Bin.Y];
                GL.Color3(TransferFunction(val));
                GL.Vertex2(x_coord, Bin.Y - 2);

                val = Bin.array[x_coord + (Bin.Y - 1) * Bin.X + layerNumber * Bin.X * Bin.Y];
                GL.Color3(TransferFunction(val));
